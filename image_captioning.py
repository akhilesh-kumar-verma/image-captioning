...
Original file is located at
    https://colab.research.google.com/drive/1gB6m1W8D6XPJe9Q_iroP531vyQBsTB1L

Image captioning with adversarial learning and attention
...
# Shape of the vector extracted from InceptionV3/resnet152 is (64, 2048)
...
# Use map to load the numpy files in parallel
dataset_val = dataset.map(lambda item1, item2: tf.numpy_function(
          map_func, [item1, item2], [tf.float32, tf.int64]),
          num_parallel_calls=tf.data.AUTOTUNE)
...
def make_discriminator_model(vocab_size, embedding_dim):
...
# Use untrained discriminator to classify generated image as real or fake
discriminator = make_discriminator_model(tokenizer.vocabulary_size(), embedding_dim)
...
# Optimizers for training generator and discriminator models
...
@tf.function
def train_step_adv(img_tensor, target):
    # loss = 0
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
      ...
      for i in range(1, target.shape[1]):
          # passing the features through the decoder
          predictions, hidden, _ = decoder(dec_input, features, hidden)
          generated_cap.append(predictions)
          # tf.concat([generated_cap, predictions], 0)
      ...
      print(target.shape)
      generated_Cap = tf.transpose(generated_cap)
      print(len(generated_cap))
      print(len(generated_cap[0]))

    #   real_output = discriminator(target, training=True)
    #   fake_output = discriminator(generated_cap, training=True)

    #   gen_loss = generator_loss(fake_output)
    #   disc_loss = discriminator_loss(real_output, fake_output)

    # generator_trainable_variables = encoder.trainable_variables + decoder.trainable_variables
...
for epoch in range(start_epoch, EPOCHS):
    ...
    for (batch, (img_tensor, target)) in enumerate(dataset):
        ...
        # print(target.shape)
        train_step_adv(img_tensor, target)
...
"""TEST MODEL AFTER ADVERSARIAL TRAINING"""
